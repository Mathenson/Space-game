...
contract SpaceRanger is ERC1155, Ownable, ERC1155Burnable, ShipHelper, Utils {
    ...
    constructor()
        ERC1155(
            "https://bafybeibxyhdne4x3uqblljkl2aetxvldtpb4lctstjqe22yr77gnrnlaia.ipfs.nftstorage.link/{id}.json"
        )
    {}
    ...
    function mint(uint8 _shipTypeId) public {
        ...
        require(
            userShips[msg.sender].length == 0,
            "You already have SpaceShip"
        );

        uint8 _shipTypeIndex = _shipTypeId - 1;
        require(
            mintedShips[_shipTypeIndex] < SHIPS_TYPE_SUPPLY,
            "No more ships of this modification in our station."
        );

        uint _id = ++totalMintedShips;

        // transfer NFT
        string memory _shipTypeLevel = Utils.concatStrings(
            Strings.toString(_shipTypeId),
        ...
        (
            ...
            uint8 _speed,
            uint8 _weapons
        ) = ShipHelper.getShipStats(_shipTypeId);
    ...
    function upgradeShipCharacteristics(
        uint _id,
        ShipUpgradeType _upgradeType
    ) public {
        Ship storage ship = ships[_id];

        require(ship.id == _id, "SpaceShip not found");
        require(
            ship.upgrades.length < 6,
            "Can't upgrade ship, spots limit reached"
        );
        ...
        if (_upgradeType == ShipUpgradeType.Armor) {
            require(
                _countUpgrades < 3,
                "Can't upgrade, armor spots limit reached"
            );
            ship.health += 2;
        } else if (_upgradeType == ShipUpgradeType.Engine) {
            require(
                _countUpgrades == 0,
                "Can't upgrade, engine spots limit reached"
            );
            ship.speed += 1;
        } else if (_upgradeType == ShipUpgradeType.Weapon) {
            require(
                _countUpgrades < 2,
                "Can't upgrade, weapon spots limit reached"
            );
            ship.attack += 2;
        } else if (_upgradeType == ShipUpgradeType.Energy) {
            require(
                _countUpgrades < 2,
                "Can't upgrade, energy spots limit reached"
            );
            ship.maxEnergy += 2;
        }
    ...
    function upgradeShipLevel(uint _id) public {
        ...
        // Burn old ship NFT
        string memory _oldShipTypeLevel = Utils.concatStrings(
            Strings.toString(ship.shipType),
            Strings.toString(ship.level)
        );
        _burn(msg.sender, Utils.stringToUint(_oldShipTypeLevel), 1);

        // Mint new NFT
        string memory _newShipTypeLevel = Utils.concatStrings(
            Strings.toString(ship.shipType),
            Strings.toString(_nextLevel)
        );
